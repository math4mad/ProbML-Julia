...
数据点的协方差矩阵已知(为了简化过程),  利用数据点和先验二元高斯分布推断后验均数向量
 ...
 流程是设立先验分布, 然后根据数据点对正态分布的参数(这里是均值)做出后验最大化采样

```
@model function gaussian_model(x)
    if ys === missing
        # Initialize `x` if missing
        x = Vector{Float64}(undef, 2,10)
    end
...
function plot_p2( z)
    z2 = z
    Σy1 = 0.1 * [2 1; 1 1]
    dist = MvNormal(z2, Σy1)
    zs = [pdf(dist, [x, y]) for x in xs, y in ys]
    ax = Axis(fig[1, 2], limits=(-1, 1, -1, 1), title="no data prior")
    contour!(ax, xs, ys, zs;)
...
function plot_p3(z)
    ...
    ax = Axis(fig[1, 3], limits=(-1, 1, -1, 1), title="post  in 10 observation")
    contour!(ax, xs, ys, zs;)
    scatter!(ax, X, size=14;)
    scatter!(ax, [0.5], [0.5]; marker=:xcross, markersize=24)
end
...
bayesian_infer(missing)|>plot_p2  #先验分布


bayesian_infer(X)|>plot_p3       #后验分布和数据

fig
#save("./inferring-unknown-vector-figure3.9-no-observation.png",fig)
